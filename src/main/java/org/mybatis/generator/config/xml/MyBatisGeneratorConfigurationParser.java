...
public class MyBatisGeneratorConfigurationParser {
    ...
    private void parseTable(Context context, Node node) {
        ...
        for (int i = 0; i < nodeList.getLength(); i++) {
            ...
            } else if ("defaultValue".equals(childNode.getNodeName())) { //$NON-NLS-1$
                parseDefaultValue(tc, childNode);
            }
        }
    }

    private void parseDefaultValue(TableConfiguration tc, Node node) {
        Properties attributes = parseAttributes(node);
        String columnName = attributes.getProperty("columnName"); //$NON-NLS-1$
        String columnValue = attributes.getProperty("columnValue"); //$NON-NLS-1$
        String dataType = attributes.getProperty("dataType"); //$NON-NLS-1$
        ...
        if (stringHasValue(columnValue)) {
            df.setColumnValue(columnValue);
        }
        if (stringHasValue(dataType)) {
            df.setDataType(dataType);
        }
        tc.addDefaultValues(df);
    }
